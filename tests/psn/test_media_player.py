...
from .conftest import DUID_1, DUID_2, make_client
...
async def test_turn_on_calls_can(
    hass: HomeAssistant, config_entry, mock_can_client
) -> None:
    await _setup(hass, config_entry, mock_can_client)
    with patch("custom_components.psn.media_player.CANClient", return_value=mock_can_client), \
...
async def test_turn_off_calls_can(
    hass: HomeAssistant, config_entry, mock_can_client
) -> None:
    await _setup(hass, config_entry, mock_can_client)
    with patch("custom_components.psn.media_player.CANClient", return_value=mock_can_client), \
...
async def test_play_media_calls_can(
    hass: HomeAssistant, config_entry, mock_can_client
) -> None:
    await _setup(hass, config_entry, mock_can_client)
    with patch("custom_components.psn.media_player.CANClient", return_value=mock_can_client), \
        ...
        await hass.services.async_call(
            "media_player",
            "play_media",
            {"entity_id": ENTITY_ID, "media_content_id": "PPSA01234_00", "media_content_type": "game"},
            blocking=True,
        )
...
async def test_can_error_logged(
    hass: HomeAssistant, config_entry, mock_can_client
) -> None:
    await _setup(hass, config_entry, mock_can_client)
    ...
    with patch("custom_components.psn.media_player.CANClient", return_value=failing_client), \
         patch("custom_components.psn.coordinator.CANClient", return_value=mock_can_client):
